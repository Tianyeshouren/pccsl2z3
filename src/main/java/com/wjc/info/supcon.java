...
public class supcon implements  ccslcon {
    String type = "supremum";
    String consclock1;
    String consclock2;
    String defclock;
    public supcon(){
    ...
    public void getText() {
        ...
        info.append(this.defclock+" = " +this.consclock1 +" v "+this.consclock2);
    ...
    public void genSMT(z3pycode z){
        ...
        sb.append("ForAll([x],Implies(And(x>=1,x<=b),Implies(H"+consclock1+"(x) >= H"+consclock2+"(x),H"+defclock+"(x)==H"+consclock2+"(x) )  ))");
        z.global_def.add(sb.toString());
        sb.delete(0,sb.length());
        sb.append("ForAll([x],Implies(And(x>=1,x<=b),Implies(H"+consclock2+"(x) >= H"+consclock1+"(x),H"+defclock+"(x)==H"+consclock1+"(x) )  ))");
        z.global_def.add(sb.toString());
    }
