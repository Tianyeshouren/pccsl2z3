...
public class intcon implements  ccslcon {
    ...
    public void genSMT(z3pycode z) {
        ...
        sb.append("ForAll([x],Implies(And(x>=1, x<=b),Implies(And(T"+consclock1+"(x),T"+consclock2+"(x)),T"+defclock+"(x))))");
        z.global_def.add(sb.toString());
        sb.delete(0,sb.length());
        sb.append("ForAll([x],Implies(And(x>=1, x<=b),Implies(T"+defclock+"(x),And(T"+consclock1+"(x),T"+consclock2+"(x)))))");
        z.global_def.add(sb.toString());
    }
}
